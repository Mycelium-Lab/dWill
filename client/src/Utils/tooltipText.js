export const tooltipText = {
    ...
    If you create a dWill with restrictions, then at the time the dWill is activated, the heir dWill be able to pick up no more than the number of tokens you have specified.
    A separate dWill is created for each token type.
    
    For example:
    If you had 1000 USDC in your account at the time the dWill was activated and created a dWill with 500 USDC limits, then your heir dWill be able to collect 500 USDC only. If you did not specify a limit when you created your dWill, your heir dWill take 1,000 USDCs.`,
    network: `
    Each dWill only works within the network on which it was created. In order to inherit tokens on different networks, you need to create a separate certificate for each of them.
    For example:
    If you want to bequeath USDC on Ethereum and Polygon networks, you need to create a dWill on the Ethereum network, then switch dWill to the BSC network and create another dWill.
    `,
    ...
    approve: `In order to create a dWill, you need to perform 2 transactions: Approve and Create a new dWill`,
